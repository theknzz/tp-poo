...
void mostra_mapa(int alt, int larg,vector<Celula> mapa) {
	...
	int a, l, i, j;
	for (a = 0;a < alt; a++) {
		if (tipoCor == 0)  //define a primeira cor a usar na linha, tendo em conta se � par ou impar
			claro = 0;
		else
			claro = 1;
		for (i = 0;i < 2;i++) {						//prepara para escrever a mesma linha 2x, uma vez que cada celula � representada em 2x2
			for (l = 0;l < larg; l++) {
				...
				for (j = 0;j < 2;j++) {				//prepara para escrever a mesma c�lula 2x, uma vez que cada c�lula � representada em 2x2
		...
		if (tipoCor == 0)							//troca tipoCor, o que definir� a cor inicial da pr�xima linha para manter padrao xadrez
			tipoCor = 1;
		else
			tipoCor = 0;
	}
